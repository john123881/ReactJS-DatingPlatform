...
export default function ParticipantModal({ participants, modalId }) {
  const {
  ...
  const handleProfileClick = async (userId) => {
    ...
    await resetAndCloseParticipantModal();
    if (participantModalRef.current) {
        participantModalRef.current.close(); // 確保模態窗口關閉
    }
        ...
        >
          <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
            <h3 className="font-black text-2xl text-white tracking-tighter uppercase italic">
              Participants <span className="text-neongreen text-sm not-italic ml-1 opacity-70">/ 參與者</span>
            </h3>
            <form method="dialog">
            ...
            {participants && participants.length > 0 ? (
              participants.map((user, index) => (
                ...
                <div className="flex flex-col items-center justify-center h-[300px] text-gray-500 gap-2">
                    <p>目前還沒有人參加這個活動</p>
                    <p className="text-xs italic">快來成為第一個參加者吧！</p>
                </div>
            )}
          </ul>
        </div>
